...
public class EventService {
    ...
    public Event ajouterEvent(Event event, MultipartFile file) {
        ...
        try {
            ...
            String url = objectStorageService.saveFileAlone(file, FOLDER);
            event.setImage(url);
        } catch (Exception e) {
    ...
    public boolean supprimerEvent(Event event) {
        if (eventRepository.findById((int) event.getID()).isPresent()) {
            if(event.getImage()!=null)
             objectStorageService.deleteFile(event.getImage());
    ...
    public Event modifierEvent(Event event, MultipartFile file) {
        if (eventRepository.findById((int) event.getID()).isPresent()) {
            if (file != null) {
                System.out.println("mdoifying in file");
                    // try {
                    // Files.createDirectories(this.root);
                    // } catch (IOException e) {
                    // throw new RuntimeException("Could not initialize folder for upload!");
